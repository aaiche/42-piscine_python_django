...
def battle_A(request, moviemon):
    ...
    if is_catched:
        ...
        del game.moviemons[moviemon.imdbID]
...
def moviedex(request):
    ...
    name = list_pokemon[game.slot].imdbID if len(list_pokemon) > 0  else ""
...
def moviedex_down(request):
    ...
    list_pokemon = [m for m in game.moviedex.values()]
    if 15 - len(game.moviemons) == game.slot:
        game.slot -= 1
    game.dump()
    name = list_pokemon[game.slot].title if len(list_pokemon) > 0 else ""
    return redirect('/moviedex')
...
def details(request, moviemon):
    ...
    for i, elem in game.moviedex.items():
        if moviemon == elem.imdbID:
            moviemon_index = i
            break
    else:
        return HttpResponseNotFound('<h1>Page not found</h1>')
    return render(request, 'game/details.html', {"moviemon": game.moviedex[i]})
...
def get_worldmap_params(game, has_moved):
    ...
    if action == "movieball":
        ...
        moviemon_name = game.get_random_movie().imdbID
    game.dump()
    return {
